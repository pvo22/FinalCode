...
public class DefectDateServiceImpl implements DefectDateService {
	...
	public List<DefectDate> getDefectDateByNameProject(String nameProject) {
		...
		for (int i = 0; i < issues.length(); i++) {
			...
			try {
				...
				for (int j = 0; j < 28; j++) {
					...
					dateobject.put("Date", currentdate.minusDays(j).toString());
			...
			if (histories.length() != 0) {
				...
				if (temporarydate[1] != null) {
					...
					for (int j = n - 1; j >= 0; j--) {
						for (k = l; k < 28; k++) {
							JSONObject dateobject = new JSONObject();
							dateobject.put("Date", currentdate.minusDays(k).toString());
		...
		List<DefectDate> listDefectDate = new ArrayList<DefectDate>();
		int j = 27;

		for (int i = 27; i >= 0; i--) {
			if ((json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalOpenedByDate") == 0)
					&& (json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalResolvedByDate") == 0)
					&& (json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalClosedByDate") == 0)) {
				continue;
			}
			DefectDate defectdate = new DefectDate();
			defectdate.setNameProject(nameProject);
			defectdate.setDate(json.getJSONArray(nameProject).getJSONObject(i).getString("Date"));
			defectdate.setTotalOpened(json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalOpenedByDate"));
			defectdate.setTotalResolved(json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalResolvedByDate"));
			defectdate.setTotalClosed(json.getJSONArray(nameProject).getJSONObject(i).getInt("TotalClosedByDate"));
			listDefectDate.add(27 - j--, defectdate);
		}

		return listDefectDate;
	}
